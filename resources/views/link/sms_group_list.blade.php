...
function emailfunction(id,value){
  ...
  if(emailValues == ''){
    ...
    emailValues = emailValues.slice(0,-2);
      window.opener.document.forms['element_create_form'].elements['sms_group'].value=emailValues;
    ...
    <div ng-app="responsibleConcernApp" ng-controller="responsibleConcernCtrl">
      ...
      <table>
        ...
        <tr ng-repeat="x in names | filter:search">
          <td id="@{{x.group_row_id}}" onclick="emailfunction(this.id,'add');" style="padding:10px;cursor:pointer;">@{{ x.group_row_id }},@{{ x.group_name }}</td>
        </tr>
...
app.controller('responsibleConcernCtrl', function($scope, $http) {
   $http.get('/phoenix/public/sms_group_api')
   .then(function (response) {$scope.names = response.data.sms_group;});
});
</script>
