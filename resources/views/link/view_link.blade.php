...
<div class="container-fluid" id="incident_main_div">	
	<h2 class="page-title">
		<a href="#" onClick="toggle_nav()" style="color:#fff;"><i class="fa fa-plus-square" ></i></a>
		View Link <span class="fw-semi-bold"></span></h2>	
		<div clss="row">
					...
					<div class="col-md-12">
						<table id="incident_search_table">
							...
							<tr>
								...
								<td colspan="10" id="section_header">
								Link</td>
							</tr>
							<!-- <tr>
	...
	<div id="incident_div" style="width: 100% !important">
			<div class="pagination"> {!! str_replace('/?', '?', $link_lists->appends(Input::except('page'))->render()) !!} </div>
				<table id="incident_table">
					...
					<tr>
						...
						<th>Vendor</th>
						<th>Client Owner</th>
						<th>Service Type NTTN</th>
						<th>Service Type Gateway</th>
						<th>Link Name NTTN</th>
						<th>Link Name Gateway</th>
						<th>Redundancy</th>
						<th>Service Impact</th>
						<th>Capacity Nttn</th>
						<th>Capacity Gateway</th>
						<th>Last Mile Provided by</th>
						<th>Last Lile Link ID</th>
						<th>Sub Center Primary</th>
						<th>Su Center Secondary</th>
						<th>Metro</th>
						<th>LH</th>
						<th>Dark Core</th>
						<th>Mobile Backhaul</th>
					...
					<tr>
						...
						<td>{{$link_list->link_category}}</td>
						<td>{{$link_list->link_conn_type}}</td>
						<td>{{$link_list->district}}</td>
						<td>{{$link_list->region}}</td>
						<td>{{$link_list->sms_group}}</td>
						<td>{{$link_list->vendor}}</td>
						<td>{{$link_list->client_owner}}</td>
						<td>{{$link_list->service_type_nttn}}</td>
						<td>{{$link_list->service_type_gateway}}</td>
						<td>{{$link_list->link_name_nttn}}</td>
						<td>{{$link_list->link_name_gateway}}</td>
						<td>{{$link_list->redundancy}}</td>
						<td>{{$link_list->service_impact}}</td>
						<td>{{$link_list->capacity_nttn}}</td>
						<td>{{$link_list->capacity_gateway}}</td>
						<td>{{$link_list->last_mile_provided_by}}</td>
						<td>{{$link_list->last_mile_link_id}}</td>
						<td>{{$link_list->sub_center_primary}}</td>
						<td>{{$link_list->sub_center_secondary}}</td>
						<td>{{$link_list->metro}}</td>
						<td>{{$link_list->LH}}</td>
						<td>{{$link_list->dark_core}}</td>
						<td>{{$link_list->mobile_backhaul}}</td>
						<td>{{$link_list->telegram_group}}</td>
						<td>{{$link_list->uni_nni}}</td>
						<td>{{$link_list->flag}}</td>
