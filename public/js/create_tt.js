...
function tt_create_form_submit(){
	...
	// var fault_problems_pair = document.getElementById("fault_chosen_probelms").value;
	// var fault_problems_array = fault_problems_pair.split("$");

	// var problems_array = new Array();

	// for(var i =0; i<fault_problems_array.length; i++){
	// 	var pair_array = fault_problems_array[i].split("#"); 
	// 	problems_array[pair_array[0]] = pair_array[1];
	// } 
	...
	for(var i=1;i<(divCounter+1);i++){
		...
		// -------------------- event time can not be more than 15 minutes ahead of current time ------------------------

		var event_time_val = document.getElementsByName('event_time_'+i)[0].value;
		var max_allowed_time = document.getElementById("max_allowed_event_time").value;

		// alert(event_time_val);
		// alert(max_allowed_time);

		var etime = new Date(event_time_val);
		...
		// var problems = document.getElementsByName('problem_category_'+i)[0].value;
		// var previously_set_problem = fault_problems_array[i].split("#");

		// if(problems != ""){
		// if(problems != pair_array[i]){
		// 	alert("You are being too smart!!! i am dissappointed !!!! :(");
		// }
		// } 
...
/***********************************************************************************************************/
	...
	hiddenInputElementGenerator('chosen_problem_category_','chosen_problem_category_',dditTableBody);
	...
	inputElementGenerator('Subcenter Name','subcenter_',dditTableBody);
	linkTypeSelectElementGenerator(dditTableBody);


	//////////////// @Ahnaf ///////////////////////////

	///////// UNMS Ticket ID Field  ////////////
	// var tr = elementGenerator('','','','tr','');
	// var label_td = elementGenerator('','','','td','');
	// var input_td = elementGenerator('','','','td','');

	// var UNMSTextNode = document.createTextNode('UNMS ID');
	
	// var input = document.createElement("input");
	// input.type = "number";
	// input.className = "form-control input-transparent";
	// input.name = "unms_id_"+divCounter;


	// label_td.appendChild(UNMSTextNode);
	// input_td.appendChild(input);

	// tr.appendChild(label_td);
	// tr.appendChild(input_td);

	// dditTableBody.appendChild(tr);var tr = elementGenerator('','','','tr','');
	// var label_td = elementGenerator('','','','td','');
	// var input_td = elementGenerator('','','','td','');
	...
	inputElementGenerator('UNMS ID','unms_id_',dditTableBody);
...
/**************Header END************************************/
    ...
    // Set provider = SCL
	$provider_id = "provider_"+divCounter;
	// $($provider_id).val('258');
	$provider_select = document.getElementsByName($provider_id)[0];
	$provider_select.value = "258";
	console.log("provider changed");


	// First task name changed
	var class_name = ".fault_"+divCounter+"_task_1_name";
	$(class_name).val('Escalated');
...
function addFaultTask(divCounter){
	$.getScript("js/bootstrap-datetimepicker.min.js", function(){
		...
		//this is where task default values go

		$task_id = "fault_"+divCounter+"_task_"+taskCounter+"_name";
		$task_name_select = document.getElementsByName($task_id)[0];
		$task_name_select.value = "Escalated";
...
function inputElementGenerator(textNode,id,tableObject){
	...
	if(textNode == 'UNMS ID'){
		dditTrTdTempElementTextarea.required = false;
		dditTrTdTempElementTextarea.type = "number";
	}
...
function elementScript(id){
	...
	var problem_category = document.getElementsByName('problem_category_'+id)[0].value;
	...
	if(problem_category ==''){
		alert('Please Insert Problem Category');
		return false;
	}
	window.open('ElementView?element_type='+element_type+'&client_id='+client_id+'&problem_category='+problem_category+'&id='+id,'_blank');
