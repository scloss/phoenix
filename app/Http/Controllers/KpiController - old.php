...
class KpiController extends Controller
{
	...
	public function kpi_view_fault(){
        ...
        if($report_list == 'region_wise_mttr'){
        	...
        	if($element_type_temp =='link'){
	            $query_start_part .= " , (select region from link_table where link_name_id = f.element_id) as region,";
        	...
        	if($element_type_temp =='site'){
        		$query_start_part .= " , (select region from site_table where site_name_id = f.element_id) as region,";
	            $query_end_part .= " AND (select include from reason_table r where r.reason_name=f.reason) ='Include' AND (select include from problem_category_table pct where pct.problem_name = f.problem_category)='Outage' group by region ";
        ...
        if($report_list == 'district_wise_mttr'){
        	...
        	if($element_type_temp =='link'){
	            $query_start_part .= " , (select district from link_table where link_name_id = f.element_id) as district , ";
        	...
        	if($element_type_temp =='site'){
        		$query_start_part .= " , (select district from site_table where site_name_id = f.element_id) as district , ";
	            $query_end_part .= " AND (select include from reason_table r where r.reason_name=f.reason) ='Include' AND (select include from problem_category_table pct where pct.problem_name = f.problem_category)='Outage' group by district ";
        ...
        if($report_list == 'client_wise_mttr'){
        	...
        	if($element_type_temp =='link'){
	            $query_start_part .= " , (select client_name from phoenix_tt_db.client_table where client_id = f.client_id) as client,client_id, ";
    ...
    public function kpi_view(){
        ...
        if($fault_id_lists !=''){
            ...
            $select_query_raw = "Select *,(select dept_name from hr_tool_db.department_table where dept_row_id = task_assigned_dept) as department from phoenix_tt_db.task_table where fault_id IN (".trim($fault_id_lists,',').") and task_status='Closed' and task_end_time_db is not null";
